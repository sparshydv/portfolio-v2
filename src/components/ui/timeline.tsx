...
function formatTimestamp(timestamp: string | Date): string {
  ...
  if (typeof timestamp === "string" && !timestamp.includes("T")) return timestamp; // fallback for simple string dates like "Day 1"
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            ...
            {showTimestamps &&
              timestampPosition === "top" &&
              item.timestamp && (
                <time className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">
            ...
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-semibold text-lg leading-tight text-[#111]">{item.title}</h3>
              {showTimestamps &&
